...
# --- Exportacion PDF (Adaptado a ReportLab para HACT) -------------------------
...
def _generar_pdf(
    ...
    # LOGO
    ruta_logo = "assets/images/logo_hact_tolima_grande.webp"
    ...
    # ENCABEZADO
    story.append(Paragraph("HOSPITAL DE ALTA COMPLEJIDAD DEL TOLIMA GRANDE", estilo_titulo))
    ...
    def _tabla_top5_pdf(df_src: pd.DataFrame, col: str, titulo: str, excluir: str = None):
        ...
        # 1. Creamos una lista temporal para agrupar los elementos
        bloque = []

        # 2. Agregamos el título al bloque en lugar de directamente a 'story'
        ...
        t.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1d4ed8")), # Azul Tolima
        ...
        # 3. Agregamos la tabla al bloque
        bloque.append(t)

        # 4. Agregamos el bloque completo a 'story' usando KeepTogether
    ...
    story.append(Paragraph(
        f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')} — HOSPITAL DE ALTA COMPLEJIDAD DEL TOLIMA GRANDE",
...
def mostrar_resumen_ejecutivo():
    ...
    with st.expander("⚙️ CONTROL DE PERÍODO", expanded=True):
        modo = st.radio(
            ...
            ["Por Mes", "Por Año"],
            index=0, horizontal=True, key="res_tol_modo",
        )
        anos = [hoy.year, hoy.year - 1, hoy.year - 2]

        if modo == "Por Mes":
            c1, c2 = st.columns(2)
            ano     = c1.selectbox("Año", anos, key="res_tol_ano_m")
            mes_nom = c2.selectbox("Mes", list(MESES.values()), index=hoy.month - 1, key="res_tol_mes_m")
            ...
            ano      = st.selectbox("Año", anos, key="res_tol_ano_a")
            f0       = f"{ano}-01-01"
    ...
    # Conexión a la base de datos de Tolima (HACT)
    try:
        conn = st.connection(
            "hact", 
            type="sql",
    ...
    meta_per = obtener_meta_guardada_tolima() * factor_meta
    ...
    nombre_archivo = f"Resumen_Ejecutivo_HACT_{etiqueta.replace(' ', '_').replace('/', '-').replace(':', '')}.pdf"
