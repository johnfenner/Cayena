...
def _generar_pdf(
    ...
    # LOGO UCM
    ruta_logo = "assets/images/logo_ucm_magdalena_sas.png"
    ...
    # ENCABEZADO
    story.append(Paragraph("UNIDAD CLÍNICA LA MAGDALENA", estilo_titulo))
    ...
    story.append(Paragraph(
        f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')} — UNIDAD CLÍNICA LA MAGDALENA",
...
def mostrar_resumen_ejecutivo():
    ...
    with st.expander("⚙️ CONTROL DE PERÍODO", expanded=True):
        modo = st.radio(
            ...
            ["Por Mes", "Por Año"],
            index=0, horizontal=True, key="res_ucm_modo",
        )
        anos = [hoy.year, hoy.year - 1, hoy.year - 2]

        if modo == "Por Mes":
            c1, c2 = st.columns(2)
            ano     = c1.selectbox("Año", anos, key="res_ucm_ano_m")
            mes_nom = c2.selectbox("Mes", list(MESES.values()), index=hoy.month - 1, key="res_ucm_mes_m")
            ...
            ano      = st.selectbox("Año", anos, key="res_ucm_ano_a")
            f0       = f"{ano}-01-01"
    ...
    # Conexión a la base de datos de UCM
    try:
        conn = st.connection(
            "ucm", 
            type="sql",
    ...
    # Meta de La Magdalena
    factor_meta = 12 if modo == "Por Año" else 1
    meta_per = obtener_meta_guardada() * factor_meta
    ...
    nombre_archivo = f"Resumen_Ejecutivo_UCM_{etiqueta.replace(' ', '_').replace('/', '-').replace(':', '')}.pdf"
