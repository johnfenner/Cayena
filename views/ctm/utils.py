...
# ==========================================
# 0. PERSISTENCIA DE CONFIGURACIÓN TRAUMANORTE
# ==========================================
ARCHIVO_META_TRAUMANORTE = "config_meta_traumanorte.json"
META_INICIAL_TRAUMANORTE = 9000000000

def obtener_meta_guardada_traumanorte():
    """Lee la meta del archivo JSON. Si no existe, lo crea con el valor inicial."""
    if not os.path.exists(ARCHIVO_META_TRAUMANORTE):
        guardar_nueva_meta_traumanorte(META_INICIAL_TRAUMANORTE)
        return META_INICIAL_TRAUMANORTE
    
    try:
        with open(ARCHIVO_META_TRAUMANORTE, 'r') as file:
            datos = json.load(file)
            return datos.get("meta_mensual", META_INICIAL_TRAUMANORTE)
    except Exception:
        return META_INICIAL_TRAUMANORTE

def guardar_nueva_meta_traumanorte(nueva_meta):
    """Sobreescribe el archivo JSON con el nuevo valor de la meta."""
    try:
        with open(ARCHIVO_META_TRAUMANORTE, 'w') as file:
...
def generar_pdf_fidedigno(tabla, fig, periodo, total_rango, meta_acumulada, cumplimiento, 
    ...
    pdf.cell(0, 10, "CLÍNICA TRAUMANORTE", ln=True, align="C")
