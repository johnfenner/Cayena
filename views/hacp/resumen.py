...
# --- Exportacion PDF (Adaptado a ReportLab para HACP) -------------------------
...
def _generar_pdf(
    ...
    # LOGO (Opcional, si lo quieres en el PDF)
    ...
    # ENCABEZADO
    story.append(Paragraph("HOSPITAL DE ALTA COMPLEJIDAD DEL PUTUMAYO", estilo_titulo))
    ...
    def _tabla_top5_pdf(df_src: pd.DataFrame, col: str, titulo: str, excluir: str = None):
        ...
        t.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1d4ed8")), # Azul Putumayo
    ...
    story.append(Paragraph(
        f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')} — HOSPITAL DE ALTA COMPLEJIDAD DEL PUTUMAYO",
...
def mostrar_resumen_ejecutivo():
    ...
    with st.expander("⚙️ CONTROL DE PERÍODO", expanded=True):
        modo = st.radio(
            ...
            ["Por Mes", "Por Año"],
            index=0, horizontal=True, key="res_putu_modo",
        )
        anos = [hoy.year, hoy.year - 1, hoy.year - 2]

        if modo == "Por Mes":
            c1, c2 = st.columns(2)
            ano     = c1.selectbox("Año", anos, key="res_putu_ano_m")
            mes_nom = c2.selectbox("Mes", list(MESES.values()), index=hoy.month - 1, key="res_putu_mes_m")
            ...
            ano      = st.selectbox("Año", anos, key="res_putu_ano_a")
            f0       = f"{ano}-01-01"
    ...
    meta_per = obtener_meta_guardada_putu() * factor_meta
    ...
    nombre_archivo = f"Resumen_Ejecutivo_HACP_{etiqueta.replace(' ', '_').replace('/', '-').replace(':', '')}.pdf"
