...
def _generar_pdf(
    ...
    estilo_titulo = ParagraphStyle("Titulo", parent=styles["Title"], fontSize=14, textColor=colors.HexColor("#1e293b"), spaceAfter=4, alignment=TA_CENTER)
    ...
    # ENCABEZADO
    story.append(Paragraph("HOSPITAL DE ALTA COMPLEJIDAD DEL MAGDALENA CENTRO", estilo_titulo))
    ...
    story.append(Paragraph(
        f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')} — HOSPITAL DE ALTA COMPLEJIDAD DEL MAGDALENA CENTRO",
...
def mostrar_resumen_ejecutivo():
    ...
    with st.expander("⚙️ CONTROL DE PERÍODO", expanded=True):
        modo = st.radio(
            ...
            ["Por Mes", "Por Año"],
            index=0, horizontal=True, key="res_dorada_modo",
        )
        anos = [hoy.year, hoy.year - 1, hoy.year - 2]

        if modo == "Por Mes":
            c1, c2 = st.columns(2)
            ano     = c1.selectbox("Año", anos, key="res_dorada_ano_m")
            mes_nom = c2.selectbox("Mes", list(MESES.values()), index=hoy.month - 1, key="res_dorada_mes_m")
            ...
            ano      = st.selectbox("Año", anos, key="res_dorada_ano_a")
            f0       = f"{ano}-01-01"
    ...
    meta_per = obtener_meta_guardada_dorada() * factor_meta
    ...
    nombre_archivo = f"Resumen_Ejecutivo_HACMC_{etiqueta.replace(' ', '_').replace('/', '-').replace(':', '')}.pdf"
